...
function formatShareInstrumentName(name) {
  const aliases = {
    "Enstie X校正箱": "EnSite X 校正箱",
    "Enstie P校正箱": "EnSite P 校正箱",
  };
  return aliases[name] || String(name || "");
}
...
function buildBookingShareText(payload) {
  const booked = payload.instruments.filter((item) => item.requestType === "booking").map((item) => formatShareInstrumentName(item.name));
  const waitlisted = payload.instruments.filter((item) => item.requestType === "waitlist").map((item) => formatShareInstrumentName(item.name));
  return [
    "【儀器借用通知】",
    `📅 日期：${formatShareDate(payload.date)}`,
    `🏥 醫院：${payload.hospital}`,
    `👤 借用人：${payload.borrower}`,
    `📝 手術內容：${payload.notes || "無"}`,
    `📦 借用儀器：${booked.join("、") || "無"}`,
    `📦 備取儀器：${waitlisted.join("、") || "無"}`,
    "",
    "✅ 已完成借用申請，系統已自動更新。",
  ].join("\n");
}
...
function buildCancelShareText(group, result) {
  const instruments = group.instruments.map((item) => formatShareInstrumentName(item.instrumentName)).filter(Boolean);
  const promotions = Array.from(new Set((result.promotions || []).map((item) => item.hospital).filter(Boolean)));
  return [
    "【儀器取消借用通知】",
    `📅 借用日期：${formatShareDate(group.date)}`,
    `🏥 醫院：${group.hospital}`,
    `👤 借用人：${group.borrower}`,
    `❌ 取消借用：${instruments.join("、") || "無"}`,
    `🏥 遞補備取醫院：${promotions.join("、") || "無"}`,
    "✅ 已完成取消作業，系統已自動更新。",
  ].join("\n");
}

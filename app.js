...
const APP_VERSION_LABEL = "V1.1.14 Beta";
const RELEASE_NOTES = [
  "修正取消借用後月行程表未同步釋出的問題。",
...
function renderProfile(profile) {
  ...
  els.profileEmail.textContent = currentProfile.email || "未取得公司帳號";
  if (currentProfile.isAdmin) {
    ...
    els.profilePermission.textContent = `可借醫院 ${currentProfile.hospitals.length} 項・區域：${currentProfile.regions.join("、")}。`;
  }
}

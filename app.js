...
const APP_VERSION_LABEL = "V1.1.6 Beta";
const RELEASE_NOTES = [
  "修正取消借用後月行程表未同步釋出的問題。",
...
function runServer(functionName, ...args) {
  return new Promise((resolve, reject) => {
    google.script.run
      ...
      .withFailureHandler((error) => reject(new Error(error && error.message ? error.message : "Apps Script 執行失敗")))
      [functionName](...args);
  });
}
...
async function fetchProfile() {
  ...
  if (canUseGoogleScriptRun()) {
    ...
    if (!result.success) throw new Error(result.message || "無法取得登入資訊");
  ...
  if (!response.ok || !result.success) throw new Error(result.message || "無法取得登入資訊");
  return result.profile;
}
...
function renderProfile(profile) {
  ...
  els.profileEmail.textContent = profile.email || "未取得公司帳號";
  if (profile.isAdmin) {
    els.profilePermission.textContent = "管理者權限：可查看全部借用。";
  } else if (profile.unrestricted || (profile.hospitals || []).includes("*")) {
    els.profilePermission.textContent = "測試版目前全部開放；填寫人員權限表後會依帳號限制。";
  } else {
    els.profilePermission.textContent = `可借醫院 ${profile.hospitals.length} 項・區域：${profile.regions.join("、")}。`;
  }
}
...
async function loadMyBookings() {
  ...
  try {
    renderMyBookings(await fetchMyBookings());
  } catch (error) {
...
async function handleMyBookingsClick(event) {
  ...
  try {
    ...
    window.alert(result.message || "已取消借用。");
  } catch (error) {
...
els.refreshMyBookings?.addEventListener("click", loadMyBookings);
